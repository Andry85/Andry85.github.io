(function(jQuery) {
	...
	jQuery.extend(jQuery.ui, {
		plugin: {
			add: function(w, c, o, p) {
				var a = jQuery.ui[w].prototype; if(!a.plugins[c]) a.plugins[c] = [];
	...
	jQuery.fn.mouseInteractionDestroy = function() {
		this.each(function() {
			if(jQuery.data(this, "ui-mouse")) jQuery.data(this, "ui-mouse").destroy(); 	
		});
	}
	
	jQuery.ui.mouseInteraction = function(el,o) {
	
		if(!o) var o = {};
		this.element = el;
		jQuery.data(this.element, "ui-mouse", this);
		
		this.options = {};
		jQuery.extend(this.options, o);
		jQuery.extend(this.options, {
			handle : o.handle ? (jQuery(o.handle, el)[0] ? jQuery(o.handle, el) : jQuery(el)) : jQuery(el),
		...
		if(!this.options.nonDestructive && (o.helper == 'clone' || o.helper == 'original')) {
			...
			o.margins = {
				top: parseInt(jQuery(el).css('marginTop')) || 0,
				left: parseInt(jQuery(el).css('marginLeft')) || 0,
				bottom: parseInt(jQuery(el).css('marginBottom')) || 0,
				right: parseInt(jQuery(el).css('marginRight')) || 0
			};
			...
			if(o.helper == 'original')
				o.wasPositioned = jQuery(el).css('position');
		...
		if(jQuery.browser.msie) jQuery(this.element).attr('unselectable', 'on');
		
	}
	
	jQuery.extend(jQuery.ui.mouseInteraction.prototype, {
		plugins: {},
		currentTarget: null,
		lastTarget: null,
		timer: null,
		...
		click: function(e) {
			...
			var initFunc = function() { //This function get's called at bottom or after timeout
				jQuery(document).bind('mouseup', self.mouseup);
		...
		start: function(e) {
			...
			o.co = jQuery(a).offset(); //get the current offset
				
			this.helper = typeof o.helper == 'function' ? jQuery(o.helper.apply(a, [e,this]))[0] : (o.helper == 'clone' ? jQuery(a).clone()[0] : a);

			if(o.appendTo == 'parent') { // Let's see if we have a positioned parent
				var cp = a.parentNode;
				while (cp) {
					if(cp.style && (jQuery(cp).css('position') == 'relative' || jQuery(cp).css('position') == 'absolute')) {
						o.pp = cp;
						o.po = jQuery(cp).offset();
						o.ppOverflow = !!(jQuery(o.pp).css('overflow') == 'auto' || jQuery(o.pp).css('overflow') == 'scroll'); //TODO!
						break;	
					}
			...
			if(!o.nonDestructive) jQuery(this.helper).css('position', 'absolute');
			if(o.helper != 'original') jQuery(this.helper).appendTo((o.appendTo == 'parent' ? a.parentNode : o.appendTo)).show();
			...
			this.helperSize = { width: jQuery(this.helper).outerWidth(), height: jQuery(this.helper).outerHeight() }; //Set helper size property
			return false;
		...
		stop: function(e) {			
			...
			jQuery(document).unbind('mouseup', self.mouseup);
			...
			if(this.helper != a && !o.beQuietAtEnd) { // Remove helper, if it's not the original node
				jQuery(this.helper).remove(); this.helper = null;
			}
			
			if(!o.beQuietAtEnd) {
				//if(o.wasPositioned)	jQuery(a).css('position', o.wasPositioned);
		...
		drag: function(e) {
			...
			if (!this.opos || (jQuery.browser.msie && !e.button)) return this.stop.apply(this, [e]); // check for IE mouseup when moving into the document again
			var o = this.options;
